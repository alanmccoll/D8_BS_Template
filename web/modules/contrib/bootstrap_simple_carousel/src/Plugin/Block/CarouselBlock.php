 ...
 * )
 */
class CarouselBlock extends BlockBase implements \Drupal\Core\Plugin\ContainerFactoryPluginInterface {
    ...
    if (!empty($items)) {
      foreach ($items as &$item) {
        ...
        if (empty($image_style) || $image_style == SettingsForm::ORIGINAL_IMAGE_STYLE_ID) {
          $item->image_url = file_create_url($file->getFileUri());
        }
        else {
          $item->image_url = ImageStyle::load($image_style)
